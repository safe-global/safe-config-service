...
class Migration0047TestCase(TestMigrations):
    migrate_from = "0046_alter_chain_create_call_address_and_more"
    migrate_to = "0047_chain_zk"
    ...
    def test_new_fields(self) -> None:
        ...
        lens = Chain.objects.get(id=232)
        zk_sync = Chain.objects.get(id=324)

        self.assertEqual(mainnet.zk, False)
        self.assertEqual(lens.zk, True)
        self.assertEqual(zk_sync.zk, True)
